...
// import User from "../../react-admin-client/src/pages/user/user";
...
router.get('/manage/product/list', (req, res) => {
  ...
  ProductModel.find({})
    .then(products => {
      // console.log("商品product",products[0]);
...
//获取购物车商品
router.get('/manage/cart/list',(req,res)=>{
  const {pageNum,pageSize,username}=req.query
  UserModel.find({username})
    .then(user=>{
      var shoppingCart=user[0].shoppingCart
      res.send({status:0,data:pageFilter(shoppingCart,pageNum,pageSize)})
      // console.log("购物车",shoppingCart[0]);
...
//为用户购物车添加商品
router.post('/manage/cart/add',(req,res)=>{
  const {product,username}=req.body
  UserModel.find({username})
    .then(user=>{
      var shoppingCart=user[0].shoppingCart
      var newCart=shoppingCart.push(product)
      UserModel.updateOne({username:username},{$set:{shoppingCart:shoppingCart}})
        .then(res.send({status:0,data:shoppingCart}))
    })
})

//删除购物车商品
router.post('/manage/cart/delete',(req,res)=>{
  const {username,productId}=req.body
  UserModel.find({username})
    .then(user=>{
      var shoppingCart=user[0].shoppingCart
      var newCart=shoppingCart.filter(item=>item._id!==productId)
      UserModel.updateOne({username},{$set:{shoppingCart:newCart}})
        .then(res.send({status:0}))
    })
})

//清空购物车
router.post('/manage/cart/clear',(req,res)=>{
  const {username}=req.body
  UserModel.updateOne({username},{$set:{shoppingCart:[]}})
    .then(res.send({status:0,data:[]}))
})
